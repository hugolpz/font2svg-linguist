...
const textToSVGpho = TextToSVG.loadSync('./fonts/NotoSansCJK-Regular.ttf'); // local custom font for pho

const data = require('./data/lists-cmn.json');
console.log("JSON data.lists[0]:", data.lists[0]);
const list = data.lists[0].list.split("");
...
const styles= {
	"top": {
	  ort : {
			...
			attributes: { fill: '#000', 'font-family':'AR PL UKai HK','font-weight':'bold','font-style':'normal' } // the font-* DO NOT work
		}
...
for (let char of list) {
	...
	const svgOrt = textToSVGort.getPath(char.ort || char, style.ort || styles.top.ort);
	...
	var svg = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="`+doc.width+`" height="`+doc.height+`" style="border:1px solid #666">`
	    ...
	    + `</svg>`;
	// Print to file
	const fileName = char +'-songti.svg';
